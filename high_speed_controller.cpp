...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        Point difference;
        difference.x = waypoint.translation.x - robot_pose.getOrigin().x();
        difference.y = waypoint.translation.y - robot_pose.getOrigin().y();

        Point relative;
        relative.x = cos(robot_theta) * difference.x + sin(robot_theta) * difference.y;
        relative.y = -(sin(robot_theta) * difference.x) + cos(robot_theta) * difference.y;

        double distance = sqrt(relative.x*relative.x + relative.y*relative.y);
        double rotation = atan2(relative.y, relative.x);

        motor_command.linear.x = 0.7 * distance * cos(abs(rotation));
        motor_command.angular.z = 0.9 * rotation + 0.5 * (rotation - prevRotation);

        std::cout << "robot theta:    " << robot_theta << std::endl;
        std::cout << "waypoint theta: " << waypoint_theta << std::endl;
        std::cout << "angular:        " << motor_command.angular.z << std::endl;
